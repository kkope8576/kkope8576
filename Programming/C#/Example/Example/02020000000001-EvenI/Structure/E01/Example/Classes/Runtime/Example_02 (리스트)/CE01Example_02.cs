 ...
 * 
 * 배열 리스트 (Array List) 란?
 * - 배열 기반으로 구현 된 리스트를 의미한다. (즉, 배열은 메모리의 물리적인 순서를 기반으로 동작하기
 * 때문에 배열 리스트 또한 메모리의 물리적인 순서에 의해 각 데이터에 번호가 부여된다는 것을 알 수
 * 있다.)
 * 
 ...
 * 가능한 장점이 존재한다. (즉, 배열은 임의 접근을 지원하기 때문에 원하는 데이터에 한번에 접근 할 수 
 * 있다는 것을 의미한다.)
 * 
 * 단, 배열 리스트는 배열을 기반으로 구현되기 때문에 특정 위치에 데이터를 추가하거나 제거 할 경우
 * 데이터의 이동이 발생하는 단점이 존재한다. (즉, 데이터의 추가/제거가 빈번 할 경우 성능 저하가
 * 발생 할 수 있다는 것을 의미한다.)
 * 
 ...
 * 삭제 된다 하더라도 불필요한 데이터 이동이 발생하지 않는 장점이 존재한다. (즉, 데이터의
 * 추가/제거 과정이 배열 리스트에 비해 용이하다는 것을 알 수 있다.)
 */

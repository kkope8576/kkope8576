 ...
 * 
 * 캡슐화란?
 * - 객체가 지니는 멤버를 외부로부터 안전하게 보호하는 개념을 의미한다. (즉, 캡슐화는 접근 제어 
 * 지시자를 통해서 구현하는 것이 가능하다.)
 * 
 * 일반적으로 객체가 사용하는 데이터 중 민감한 데이터를 가능하면 외부로부터 안전하게 보호하는 것이 
 * 객체를 좀 더 안전하게 사용하는 개념이라는 것을 알 수 있다.
 * 
 * 상속이란?
 * - 클래스 간에 부모/자식 관계를 형성시키는 개념을 의미한다. (즉, 특정 클래스가 상속 관계에 
 * 놓여있다면 클래스가 간에 상/하 관계가 형성된다는 것을 알 수 있다.)
 * 
 ...
 * 
 * 다형성이란?
 * - 사물을 바라보는 시선에 따라 다양한 형태를 지니는 개념을 의미한다. (즉, 다형성을 활용하면 동일한 
 * 사물을 다른 형태로 인지하는 것이 가능하다.)
 * 
 ...
 */
namespace Example._02020000000001_EvenI.Programming.E01.Example.Classes.Runtime.Example_12
{
	/**
	 * Example 12
	 */
	internal class CE01Example_12
	{
		...
		public static void Start(string[] args)
		{
			 ...
			 * 
			 * 이는 부모 클래스에는 자식 클래스에 대한 정보가 없기 때문이다. (즉, 반대로 자식 
		...
		private class CE01Derived_12 : CE01Base_12
		{
			...
			public new void ShowInfo()
			{
				 ...
				 * 가능하기 때문에 이 경우 우선 순위는 자식 클래스 더 높다. (즉, 해당 경우 base 
				 * 키워드를 생략하면 항상 자식에 존재하는 멤버에 접근한다는 것을 알 수 있다.)
				 */
				base.ShowInfo();
		...
		private class CE01Derived_12 : CE01Base_12
		{
			 ...
			 * 순서는 반드시 부모 -> 자식 순으로 이루어져야한다. (즉, 자식 클래스는 반드시 부모 
			 * 클래스의 생성자를 호출해줘야한다.)
			 * 
			 * 부모 클래스의 생성자는 자식 클래스의 생성자에서 반드시 호출해줘야하며 이는 base 
			 * 키워드를 사용하면 가능하다.
			 * 
		...
		private class CE01Base_12
		{
			...
			/*
			 * 가상 메서드란?
			 * - 실제 호출되는 메서드가 프로그램이 실행 중에 결정되는 기능을 의미한다. (즉, 가상 
			 * 메서드를 호출했을 경우 참조하는 객체에 따라 호출 결과가 달라진다는 것을 의미한다.)
			 * 
			 * 가상 메서드는 호출 될 메서드를 실행 중에 결정하는 동적 바인딩을 지원하는 기능이라는 
			 * 것을 알 수 있다.
			 * 
			 * 자식 클래스가 부모 클래스에 존재하는 가상 메서드를 재정의 할 경우 자식 클래스의 
			 * 메서드가 부모 클래스의 메서드 대신에 호출되는 것이 가능하다. (즉, 가상 메서드를 
			 * 활용하면 특정 객체를 참조하는 자료형에 상관없이 항상 동일한 결과를 만들어내는 것이 
			 * 가능하다.)
			 */
			/** 정보를 출력한다 */

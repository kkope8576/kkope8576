...
/*
 * 제네릭이란?
 * - 자료형을 명시하지 않고 메서드 또는 클래스를 정의 할 수 있는 기능을 의미한다. (즉, 제네릭을 
 * 활용하면 여러 자료형에 동작하는 메서드 또는 클래스를 구현하는 것이 가능하다.)
 * 
 * 제네릭을 통해 정의 된 메서드 또는 클래스는 사용 될 때 자료형이 결정되는 특징이 존재한다. (즉, 
 * 메서드는 호출되는 시점에 자료형이 결정되고 클래스는 객체로 생성되는 시점에 자료형이 결정된다는 
 * 것을 알 수 있다.)
 * 
 * 따라서, 각 자료형마다 별도로 메서드 또는 클래스를 제작 할 필요없이 제네릭은 하나의 메서드 또는 
 ...
 * 
 * 제네릭 형식 인자란?
 * - 메서드 또는 클래스를 정의하는데 필요한 자료형을 임시적으로 대체하기 위한 자료형을 의미한다. 
 * (즉, 제네릭 형식 인자는 메서드 또는 클래스가 동작 할 때 실제 자료형은 치환된다는 것을 알 수 
 * 있다.)
 */
namespace Example._02020000000001_EvenI.Programming.E01.Example.Classes.Runtime.Example_14
{
	/**
	 * Example 14
	 */
	internal class CE01Example_14
	{
		 ...
		 * 
		 * C# 제네릭 메서드 또는 제네릭 클래스는 기본적으로 모든 자료형에 동작하도록 정의 될 필요가 
		 * 있다.
		 * 
		 * 따라서, 특정 자료형에만 동작하는 메서드 또는 클래스를 정의하고 싶다면 where 키워드를 

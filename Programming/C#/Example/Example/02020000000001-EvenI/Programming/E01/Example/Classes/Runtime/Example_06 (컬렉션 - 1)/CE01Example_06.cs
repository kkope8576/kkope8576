...
/*
 * 컬렉션이란?
 * - 여러 데이터를 관리 할 수 있는 기능을 의미한다. (즉, 컬렉션을 활용하면 여러 데이터를 좀 더 
 * 효율적으로 제어하는 것이 가능하다.)
 * 
 ...
 * 많아질수록 선형 컬렉션에 비해 성능이 뛰어나다는 장점이 존재한다. (즉, 적은 데이터를 관리 할 때는
 * 선형 컬렉션과 비선형 컬렉션간에 성능 차이가 거의 없다는 것을 알 수 있다.)
 * 
 * C# 주요 선형 컬렉션 종류
 * - 배열
 * - 리스트
 ...
 * 
 * 리스트란?
 * - 관리되는 데이터에 순서가 존재하는 컬렉션을 의미한다. (즉, 리스트는 배열과 같이 데이터의 순서가 
 * 존재하기 때문에 특정 위치에 데이터를 가져오기 위해서는 [] (인덱스 연산자) 와 인덱스 번호가 
 * 사용 된다는 것을 알 수 있다.)
 * 
 ...
 * 순서를 알고 있다면 [] (인덱스 연산자) 를 사용해서 한번에 접근하는 것이 가능하다. (즉, 임의 접근이 
 * 가능하다는 것을 알 수 있다.)
 * 
 ...
 * 발생한다. (즉, 배열 리스트는 데이터의 삽입/삭제가 빈번하게 발생 할 경우 프로그램의 성능이 
 * 저하된다는 것을 알 수 있다.)
 * 
 * 반면, 연결 리스트는 참조에 의해서 데이터의 순서를 만들어내기 때문에 중간에 데이터가 추가되거나 
 * 삭제 된다하더라도 데이터의 이동이 발생하지 않는다. (즉, 데이터의 삽입/삭제가 빈번하게 발생 할 

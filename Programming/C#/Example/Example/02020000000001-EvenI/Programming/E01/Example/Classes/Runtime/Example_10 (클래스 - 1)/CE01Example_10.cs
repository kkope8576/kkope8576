 ...
 */
namespace Example._02020000000001_EvenI.Programming.E01.Example.Classes.Runtime.Example_10
{
	/**
	 * Example 10
	 */
	internal class CE01Example_10
	{
		...
		private class CCharacter
		{
			...
			{
				/*
				 * 멤버 메서드는 동일한 클래스에 존재하는 다른 멤버에 접근하는 것이 가능하다. (즉, 
				 * 멤버 메서드가 호출 될 때 동일한 클래스에 존재하는 멤버는 입력 값으로 전달 할 
				 * 필요가 없다.)
				 * 
		...
		private class CCharacter
		{
			 ...
			 * 구현 할 필요가 있다. (즉, 객체가 생성되기 위해서는 반드시 생성자가 호출되어야한다는 
			 * 것을 알 수 있다.)
			 * 
			 ...
			 * 
			 * this 키워드란?
			 * - 객체 자신을 참조하는 키워드를 의미한다. (즉, 해당 키워드가 존재하기 때문에 멤버 
			 * 메서드에서는 동일한 객체에 존재하는 다른 멤버에 접근하는 것이 가능하다.)
			 * 
			 * Ex)
			 * class CSomeClass
			 * {
		 ...
		 * 멤버에 접근 할 수 있는 범위를 제한하는 것이 가능하다.)
		 * 
		 * C# 주요 접근 제어 지시자 종류

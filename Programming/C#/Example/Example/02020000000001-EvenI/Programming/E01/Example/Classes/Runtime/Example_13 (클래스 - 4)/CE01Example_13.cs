...
/*
 * 추상 클래스란?
 * - 일반적인 클래스와 달리 직접 객체화 시키는 것이 불가능한 클래스를 의미한다. (즉, 추상 클래스는 
 ...
 * 
 * 확장 메서드란?
 * - 특정 클래스가 지니고 있는 기능을 메서드의 형태로 확장 할 수 있는 기능을 의미한다. (즉, 확장 
 * 메서드를 활용하면 기존 클래스를 수정하거나 상속하지 않고도 기존 클래스에 원하는 기능을 추가하는 
 * 것이 가능하다.)
 * 
 ...
 * this 키워드를 명시함으로서 어떤 자료형의 확장 메서드라는 것을 알릴 필요가 있다. (즉, this 
 * 키워드를 명시하지 않으면 단순한 정적 메서드라는 것을 알 수 있다.)
 * 
 ...
 * 
 * 연산자 오버로딩이란?
 * - 객체를 대상으로 산술 연산자와 같은 연산자를 사용할 수 있게 해주는 기능을 의미한다. (즉, 연산자 
 * 오버로딩을 활용하면 객체를 일반적인 데이터처럼 제어하는 것이 가능하다.)
 * 
 * Ex)
 * class CSomeClass
 * {
...
namespace Example._02020000000001_EvenI.Programming.E01.Example.Classes.Runtime.Example_13
{
	...
	internal class CE01Example_13
	{
		...
		private abstract class CBase
		{
			 ...
			 * 메서드를 의미한다. (즉, 메서드 몸체가 존재하기 않기 때문에 미완성 된 메서드라는 것을 
			 * 알 수 있다.)
			 * 

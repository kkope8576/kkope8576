...
/*
 * 메서드 (함수) 란?
 * - 명령문의 일부 or 전체를 따로 때어내서 재사용 할 수 있는 기능을 의미한다. (즉, 메서드를 
 * 활용하면 중복적으로 발생하는 명령문을 줄이는 것이 가능하다.)
 * 
 * 메서드는 내부적으로 메서드가 호출 될 때 실행되는 명령문을 포함하고 있으며 필요에 따라 메서드가 
 * 동작하기 위해서 필요한 데이터를 외부로부터 입력 받는 것이 가능하다. (즉, 프로그래밍의 
 * 메서드 (함수) 는 수학의 함수와 조금 다르다는 것을 알 수 있다.)
 * 
 * C# 메서드 유형
 ...
 * 명시하는 것이 가능하다. (즉, 입력 데이터는 여러 개를 받는 것이 가능하지만 출력은 오직 하나의 
 * 데이터만 출력하는 것이 가능하다.)
 * 
...
namespace Example._02020000000001_EvenI.Programming.E01.Example.Classes.Runtime.Example_08
{
	...
	internal class CE01Example_08
	{
		...
		public static void Start(string[] args)
		{
			 ...
			 * 알 수 있다. (즉, 메서드 내부에서는 원본 변수의 값을 변경하는 것이 불가능하다는 것을 
			 * 알 수 있다.)
			 * 
			...
			char a_chOperator, int a_nValB)
		{
			 ...
			 * 명시 된 반환 값은 메서드를 호출한 곳으로 전달 된다는 특징이 존재한다. (즉, 메서드가 
			 * 반환 값이 존재 할 경우 반드시 return 키워드를 사용해서 반환 값을 명시해 줄 필요가 
			 * 있다.)
			 */
			return 0;
		}

...
namespace Example._02910000000001_EvenI.Algorithm.E01.Solution.Classes.Runtime.Solution_03
{
	/**
	 * Solution 3
	 */
	internal class CS01Solution_03
	{
		...
		public static void Start(string[] args)
		{
			...
			Console.WriteLine("\n결과 : {0}", S01FindStr_03(oStr, oPattern));
		}

		/** 문자열을 탐색한다 */
		private static int S01FindStr_03(string a_oStr, string a_oPattern)
		{
			int i = 0;

			var oTable_GoodSuffix = S01MakeTable_GoodSuffix_03(a_oPattern);
			var oTable_BadCharacter = S01MakeTable_BadCharacter_03(a_oPattern);
			...
			while(i <= a_oStr.Length - a_oPattern.Length)
			{
				...
				i += Math.Max(oTable_GoodSuffix[j + 1], j - oTable_BadCharacter[a_oStr[i + j]]);
			}
		...
		/** 착한 접미부 테이블을 생성한다 */
		private static int[] S01MakeTable_GoodSuffix_03(string a_oPattern)
		{
			...
			for(i = 0; i < a_oPattern.Length + 1; ++i)
			{
				// 거리 설정이 가능 할 경우
		...
		/** 나쁜 문자 테이블을 생성한다 */
		private static int[] S01MakeTable_BadCharacter_03(string a_oPattern)
		{

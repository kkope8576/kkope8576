...
/*
 * 트리 (Tree) 란?
 * - 데이터 간에 상/하 관계를 형성 할 수 있는 자료구조를 의미한다. (즉, 데이터 간에 부모 / 자식
 * 관계를 형성 할 수 있다는 것을 의미한다.)
 * 
 * 트리는 내부적인 구현 방식에 따라 배열 or 참조를 통해 구현하는 것이 가능하며 참조를 통해 구현되는
 * 방식에는 N 링크 방식과 오른쪽 자식 - 왼쪽 형제 방식이 존재한다. (즉, 배열 or N 링크 방식은 자식
 * 데이터의 최대 개수가 정해져있는 방식이라는 것을 의미한다.)
 * 
